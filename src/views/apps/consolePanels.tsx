...
/** Top-level aggregator: runs every audit in parallel, composes a Health
 * Score + per-category sub-scores + a global severity-sorted findings list,
 * and exposes ONE master IDE prompt + ONE Buddy handoff covering everything. */
export function HealthPanel({ onHandoff }: { onHandoff?: OnHandoff } = {}) {
  ...
  const run = useCallback(async () => {
    ...
    setError(undefined);
    try {
      // Fire every audit in parallel — order is preserved by Promise.all.
      const [errors, sec, a11y, seo, sens, vit, tech] = await Promise.all([
        runTool<{ matches: ErrorMatch[]; scanned: number; matchCount: number; hint?: string }>('analyze_errors'),
        runTool<{ url: string; tls: { https: boolean }; csp: { metaPolicy: string | null; present: boolean }; mixedContent: string[]; cookies: { total: number; flagged: { name: string; domain: string; secure?: boolean; httpOnly?: boolean; sameSite?: string; issues: string[] }[] } }>('scan_security'),
        runTool<A11yReportType>('analyze_a11y'),
        runTool<SeoReportType & { url: string; h1Text?: string }>('analyze_seo'),
        runTool<{ hits: SensitiveHit[]; scanned: number }>('scan_sensitive_data'),
        runTool<{ url: string; title: string; vitals: Record<string, { value?: number; unit: string; verdict: 'good' | 'needs-improvement' | 'poor' | 'unknown' }> }>('web_vitals'),
        runTool<{ url: string; matches: TechMatch[] }>('detect_tech_stack'),
      ]);
      // Compose every successful result; an SW error on one audit just leaves
      // that category un-audited rather than failing the whole report.
  ...
  const sendToBuddy = () => {
    ...
    // Build a compact Buddy prompt summarising the top issues across every
    // category so the agent has something concrete to start with.
    const lines: string[] = [];
